...
GOMP_parallel_start (void (*fn) (void *), void *data, unsigned num_threads)
{
	...
	gomp_team_start (fn, data, num_threads, 0, gomp_new_team (1));
}
...
GOMP_parallel (void (*fn) (void *), void *data, unsigned num_threads, unsigned int flags)
{
  num_threads = gomp_resolve_num_threads (num_threads, 0);
	...
	{
		FIRST = 0;
		ARINC_MAX_THREADS = num_threads;
		printf("first parallel block : ARINC_MAX_THREADS fixed to %d\n",ARINC_MAX_THREADS);
	}
	
	/*	if not first parallel block and more 
		threads needed than created 	*/
	if(num_threads > ARINC_MAX_THREADS) 
	{
		printf("warning: ARINC_MAX_THREADS is %d, but %d threads needed(ignored)\n", ARINC_MAX_THREADS, num_threads);
		num_threads = ARINC_MAX_THREADS;
	}
	#endif
	/* End touch A653 */
